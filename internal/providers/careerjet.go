...
// careerjetMaxPages bounds each search to the newest forty postings (the API
// serves twenty per page whatever page_size says). Results are date-sorted and
// anything older than the retention window is dropped at ingest, so a third
// page spends a request on postings we would throw away: measured, the newest
// forty already covered every posting inside the window for all eight searches.
const careerjetMaxPages = 2
